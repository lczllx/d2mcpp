...
// license: Apache-2.0
// file: src/en/cpp14/tests/01-relaxed-constexpr/1.cpp
//
// Exercise: cpp14 | 01 - relaxed constexpr | constexpr branches and local variables
//
// Tips:
// - C++14 constexpr functions can use if / switch branches
// - Multiple local variables can be declared and used in constexpr functions
//
// Docs:
...
int main() {
    ...
    static_assert(a1 == 10, "");
    d2x::check_eq(a1, 10, "my_abs(-10) == 10");

    constexpr int a2 = my_abs(0);
    d2x::check_eq(a2, 0, "my_abs(0) == 0");

    // 1. switch branch
    constexpr int d7 = days_in_month(7);
    static_assert(d7 == 31, "July has 31 days");
    d2x::check_eq(d7, 31, "days_in_month(7) == 31");

    constexpr int d2 = days_in_month(2);
    d2x::check_eq(d2, D2X_YOUR_ANSWER, "days_in_month(2) == 28");

    // 2. multiple local variables + loop — fibonacci
    constexpr int f10 = fib(10);
    static_assert(f10 == 55, "fib(10) should be 55");
    d2x::check_eq(f10, 55, "fib(10) == 55");

    constexpr int f0 = fib(0);
    d2x::check_eq(f0, 0, "fib(0) == 0");
